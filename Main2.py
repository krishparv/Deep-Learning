...
def AlignmentAttention(input_1, input_2):
    ...
    w_att_1 = Sequential()
    w_att_1.add(dot([input_1.outputs , input_2.outputs], axes=1, normalize=False))
    ...
    q1_concat = keras.layers.concatenate([input_1.output, in2_aligned.output])
    q1_combined.add(Model(inputs=[input_1.input, in2_aligned.input], outputs=q1_concat))
    q2_combined = Sequential()
    q2_concat = keras.layers.concatenate([input_2.output, in1_aligned.output])
  ...
  #  if shortcut:
     ...
     #   left_branch_aux1.add(gru1)
      #  left_branch_aux2 = Sequential()
       ...
       # left_branch_aux2.add(merged1)
        ...
        #left_branch = left_branch_aux2.add(merged1)
    #else:
   ...
   # if alignment: left_branch, right_branch = AlignmentAttention(left_branch, right_branch)
    if selfattention:
        ...
        gru3 = GRU(hidden_units, return_sequences=False)
    ...
    con_layer = Model([left_branch.input, right_branch.input], outputs=merge_con)
    ...
    mul_layer = Model([left_branch.input, right_branch.input], outputs=merge_mul)
    dif_layer = Sequential(name="dif_layer")
    dif_lambda=Lambda(lambda x: x[0]-x[1], output_shape=lambda x: x[0])([left_branch.output, right_branch.output])
    dif_layer=Model([left_branch.input, right_branch.input], outputs=dif_lambda)
   # dif_layer.add(dif_lambda)
    ...
    print(mul_layer.summary())
    print(dif_layer.summary())

    final_model = Sequential(name="final_model")
    if onlyconcat:
        merge_threeconcat=con_layer
       # final_model.add(merge_threeconcat)
    else:
...
def evaluate_deep_neural_net(dataset='dataset-string-similarity.txt', method='gru', training_instances=-1,
    ...
    Y2 = []
    start_time = datetime.now()
    print('Reading dataset... ' + str((start_time - start_time).total_seconds()*3600))
    ...
    with open(dataset) as csvfile:
        ...
        for row in reader:
            ...
            if len(XA1) < ((num_true + num_false) / 2.0):
                ...
                XA2.append(row['s1'])
                XB2.append(row['s2'])
        print("Dataset read... " + str((datetime.now()- start_time).total_seconds()*3600))
    ...
    filename1 = path.join(mkdtemp(), 'temporary-file-dnn-1-'+ method +'.dat')
   # aux1 = np.memmap("temporary-file-dnn-1-" + method, mode="w+", shape=(len(XA1), max_seq_len, len(chars)),
   #                  dtype=np.bool)
    aux1 = np.memmap(filename1, mode="w+", shape=(len(XA1), max_seq_len, len(chars)),
                     dtype=np.bool)
    for i, example in enumerate(XA1):
        for t, char in enumerate(example):
            if t < max_seq_len:
                ...
                break
    XA1 = aux1
    filename2 = path.join(mkdtemp(), 'temporary-file-dnn-2-'+ method +'.dat')
  
   # aux1 = np.memmap("temporary-file-dnn-2-" + method, mode="w+", shape=(len(XB1), max_seq_len, len(chars)),
   #                  dtype=np.bool)
    aux1 = np.memmap(filename2, mode="w+", shape=(len(XB1), max_seq_len, len(chars)),
                     dtype=np.bool)
    ...
    filename3 = path.join(mkdtemp(), 'temporary-file-dnn-3-'+ method +'.dat')
    aux1 = np.memmap(filename3, mode="w+", shape=(len(XA2), max_seq_len, len(chars)),
                     dtype=np.bool)
    for i, example in enumerate(XA2):
        for t, char in enumerate(example):
            if t < max_seq_len:
                ...
                break
    XA2 = aux1
    filename4 = path.join(mkdtemp(), 'temporary-file-dnn-4-'+ method +'.dat')
    aux1 = np.memmap(filename4, mode="w+", shape=(len(XB2), max_seq_len, len(chars)),
                     dtype=np.bool)
    for i, example in enumerate(XB2):
        for t, char in enumerate(example):
            if t < max_seq_len:
                ...
                break
    XB2 = aux1
    print("Temporary files created... " + str((datetime.now() - start_time).total_seconds()*3600))
    ...
    aux1, time1 = deep_neural_net_gru(train_data_1=XA1[0:training_instances, :, :],
                                      ...
                                      bidirectional=bidirectional,alignment=False, hidden_units=hiddenunits, n=1)
    aux2, time2 = deep_neural_net_gru(train_data_1=XA2[0:training_instances, :, :],
                                      ...
                                      bidirectional=bidirectional, alignment=False,hidden_units=hiddenunits, n=2)
...
evaluate_deep_neural_net(dataset="dataset_final_jrc_person_new.csv")
