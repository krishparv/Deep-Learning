...
from keras.layers import Dense, Dropout, GRU, Bidirectional, GlobalMaxPooling1D, Layer, Lambda, Permute
...
def deep_neural_net_gru(train_data_1, train_data_2, train_labels, test_data_1, test_data_2, test_labels, max_len,
    ...
    if shortcut:
        ...
        merged1 = Concatenate([left_branch.output, left_branch_aux1.output], axis=-1)
    ...
    merge_con = Concatenate([left_branch.output, right_branch.output], axis=-1)
    ...
    merge_mul = multiply_layer([left_branch.output, right_branch.output], axis=-1)
    ...
    if onlyconcat:
        ...
        merge_threeconcat = Concatenate([con_layer.output, mul_layer.output, dif_layer.output], axis=-1)
        final_model = Model([con_layer.input, mul_layer.input, dif_layer.input], outputs=merge_threeconcat)
